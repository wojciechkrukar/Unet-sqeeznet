...
def fire_module(x, fire_id, squeeze=16, expand=64, fire_blockname=''):
    ...
    left  = Conv2D(expand, (1, 1), padding='same', activation='relu', name=f_name.format(fire_id, "expand1x1"))(x)
    right = Conv2D(expand, (3, 3), padding='same', activation='relu', name=f_name.format(fire_id, "expand3x3"))(x)

    out = concatenate([left, right], axis=channel_axis, name=f_name.format(fire_id, "concat") + fire_blockname)
...
def fire_module_up(x1, x2, kernel_size=1, strides=(1, 1), fire_id=0, squeeze=16, expand=64):
    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1
    x1 = Conv2DTranspose(squeeze, kernel_size, strides=strides, padding='same')(x1)
    up = concatenate([x1, x2], axis=channel_axis)
    up = fire_module(up, fire_id=fire_id, squeeze=squeeze, expand=expand)
...
def SqueezeUNet(inputs, num_classes=None, include_top=True, dropout=0.0, fireplus=True, reshape_out=False, activation='sigmoid'):
    ...
    up1 = last_block
    if fireplus:
        up1 = fire_module_up(last_block, block4, kernel_size=1,strides=1, fire_id=12, squeeze=64, expand=256)

    up2 = fire_module_up(up1, block3, kernel_size=1,strides=1, fire_id=13, squeeze=48, expand=192)
    up2 = UpSampling2D(size=(2, 2))(up2)

    up3 = fire_module_up(up2, block2, kernel_size=1,strides=1, fire_id=14, squeeze=32, expand=128)
    up3 = UpSampling2D(size=(2, 2))(up3)

    up4 = fire_module_up(up3, block1, kernel_size=1,strides=1, fire_id=15, squeeze=16, expand=64 )
    up4 = UpSampling2D(size=(2, 2))(up4)

    up5 = fire_module_up(up4, conv1, kernel_size=1,strides=1, fire_id=16,  squeeze=16, expand=32 )
